...
def draw_angle_arc(frame, center, angle, color, radius=40):
    ...
    cv2.ellipse(frame, center, (radius, radius), 0, 0, int(angle), color, 2)
...
while True:
    ...
    if lms is not None:
        ...
        cv2.putText(frame, label, (18, 47), cv2.FONT_HERSHEY_SIMPLEX, 1.2, color, 3)

        # Confidence bar
        cv2.rectangle(frame, (10, 65), (230, 83), GRAY, -1)
        cv2.rectangle(frame, (10, 65), (10 + int(confidence * 220), 83), color, -1)
        draw_text_bg(
            frame, f"{confidence*100:.0f}% confidence", (10, 105), font_scale=0.6
        )

        # Tip
        draw_text_bg(
            frame, tip, (10, h - 20), font_scale=0.65, color=WHITE, bg=(20, 20, 20)
        )

    else:
        draw_text_bg(
            frame,
            "No person detected — step back",
            (20, 50),
            font_scale=0.8,
            color=YELLOW,
            bg=BLACK,
