...
def verfiy_token(cre: HTTPAuthorizationCredentials = Depends(barear_chema)):
    ...
    if decode is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="this toke is invalide"
        )
    return decode


def get_current_user(
    token: dict = Depends(verfiy_token), db: Session = Depends(get_db)
):
    email = token.get("sub")
...
def login(user: UserSchema.loginUser, db: Session = Depends(get_db)):
    ...
    if not extist_user or not pwd_context.verify(
        user.password, extist_user.hashedpassword
    ):
        raise HTTPException(
            status_code=401,
